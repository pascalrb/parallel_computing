...
int main(int argc, char **argv)
{
    ...
    if(d_1->rows != d_2->cols || d_1->cols != d_2->rows){
        printf("ERROR: Matrix dimension mismatch.\n");
        printf("The number of columns/rows of matrix A must match the number of rows/columns of matrix B.\n");
    ...
    for(int i = 0; i < d_1->rows; i++){
        for(int j = 0; j < d_2->cols; j++){
            d_3->data_point[i][j] = 0;
            for(int k = 0; k < d_2->rows; k++){
                #pragma omp critical
                {
